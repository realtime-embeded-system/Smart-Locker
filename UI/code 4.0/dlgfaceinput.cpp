...
void dlgFaceInput::on_pushButton_inpput_clicked()
{
    ...
    {
        QMessageBox::warning(NULL, QStringLiteral("hint"), "name is null", QMessageBox::Yes);
        return;
    }
    QString path = "./images/";

    cv::imwrite(path.append(imageName).append(".jpg").toStdString(),img);
    QMessageBox::information(NULL, QStringLiteral("hint"), "login complete", QMessageBox::Yes);
    close();
}
